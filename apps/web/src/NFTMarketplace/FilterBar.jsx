  ...
  return (
    <section className="nft-card rounded-2xl p-5 sm:p-6">
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr]">
        <div>
          <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-[var(--exa-text-secondary)]">Search</label>
          <div className="nft-input-wrap flex items-center rounded-xl px-4 py-3">
            <Search className="h-4 w-4 text-[var(--exa-text-muted)]" aria-hidden="true" />
            <input
              type="text"
              value={search}
              onChange={(event) => onSearch(event.target.value)}
              placeholder="Search by name, collection, creator"
              className="ml-2 w-full bg-transparent text-sm text-[var(--exa-text-primary)] outline-none placeholder:text-[var(--exa-text-muted)]"
            />
          </div>
        </div>
...
function SelectField({ label, value, onChange, options }) {
  return (
    <div>
      <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-[var(--exa-text-secondary)]">{label}</label>
      <div className="nft-input-wrap rounded-xl px-4 py-3">
        <select value={value} onChange={(event) => onChange(event.target.value)} className="w-full bg-transparent text-sm text-[var(--exa-text-primary)] outline-none">
          {options.map((option) => (
