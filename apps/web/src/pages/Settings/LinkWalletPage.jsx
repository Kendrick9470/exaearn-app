...
function LinkWalletPage({ onBack }) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[var(--exa-bg-primary)] text-white">
      ...
      <header
        className="sticky top-0 z-30 border-b border-[var(--exa-border)] bg-[var(--exa-surface-elevated)] backdrop-blur-xl"
      ...
      >
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between px-4 pb-3 pt-3 sm:px-6">
          <button
            type="button"
            onClick={onBack}
            className="rounded-xl border border-[var(--exa-border)] bg-[var(--exa-surface-hover)] p-2 text-[var(--exa-text-secondary)] shadow-[var(--exa-shadow-soft)] transition hover:border-cyan-300/55 hover:text-cyan-200"
            aria-label="Back"
          >
            <ArrowLeft className="h-5 w-5" />
          </button>
          <h1 className="text-lg font-semibold text-[var(--exa-text-primary)]">Link Wallet</h1>
      ...
      >
        <article className="mb-4 rounded-3xl border border-[var(--exa-border)] bg-[var(--exa-surface)] p-5 text-center shadow-[var(--exa-shadow-panel)] backdrop-blur-xl">
          ...
          <h2 className="text-lg font-semibold text-[var(--exa-text-primary)]">Connect Your Web3 Wallet</h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-[var(--exa-text-secondary)]">
            Securely link your wallet to access EXA rewards, token transactions, and blockchain verification.
          </p>
        </article>

        <article className="mb-4 rounded-3xl border border-[var(--exa-border)] bg-[var(--exa-surface)] p-4 shadow-[var(--exa-shadow-panel)]">
          <h3 className="mb-3 text-sm font-semibold text-[var(--exa-text-primary)]">Wallet Options</h3>
              ...
              return (
                <button
                  ...
                  className={`group flex w-full items-center justify-between rounded-2xl border px-3 py-3 text-left transition ${
                    active
                      ? "border-cyan-300/45 bg-[var(--exa-surface-hover)] shadow-[0_0_22px_rgba(34,211,238,0.22)]"
                      : "border-[var(--exa-border)] bg-[var(--exa-surface-hover)] hover:-translate-y-0.5 hover:border-[var(--exa-border)]"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <span className={`inline-flex h-10 w-10 items-center justify-center rounded-full border border-[var(--exa-border)] bg-gradient-to-br ${wallet.glow} shadow-[inset_0_0_16px_rgba(34,211,238,0.18)]`}>
                      <Icon className="h-4.5 w-4.5 text-[var(--exa-text-primary)]" />
                    </span>
                    <span>
                      <span className="block text-sm font-medium text-[var(--exa-text-secondary)]">{wallet.name}</span>
                      <span className="block text-xs text-[var(--exa-text-muted)]">{wallet.description}</span>
                    </span>
                  </div>
                  <ArrowRight className={`h-4 w-4 ${active ? "text-cyan-200" : "text-[var(--exa-text-muted)]"}`} />
                </button>
              );
            })}
          </div>
        ...
        <article className="mb-4 rounded-2xl border border-[var(--exa-border)] bg-[var(--exa-surface)] p-4">
          <ul className="space-y-2 text-sm text-[var(--exa-text-secondary)]">
        ...
        {connectedWallet ? (
          <article className="rounded-2xl border border-emerald-300/35 bg-emerald-500/10 p-4 shadow-[0_0_24px_rgba(16,185,129,0.18)]">
            ...
            <div className="mt-2 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-[var(--exa-text-primary)]">{connectedWallet.name}</p>
                <p className="text-xs text-[var(--exa-text-secondary)]">{connectedWallet.address}</p>
              </div>
      ...
      {showConnectModal ? (
        <div className="fixed inset-0 z-50 flex items-end bg-black/55 sm:items-center sm:justify-center">
          ...
          <div className="relative w-full rounded-t-3xl border border-[var(--exa-border)] bg-[var(--exa-surface)] p-5 shadow-[0_0_36px_rgba(34,211,238,0.2)] sm:max-w-sm sm:rounded-3xl">
            <h3 className="text-base font-semibold text-[var(--exa-text-primary)]">Secure Connection</h3>
            <p className="mt-1 text-sm text-[var(--exa-text-secondary)]">
      ...
      {showManualModal ? (
        <div className="fixed inset-0 z-50 flex items-end bg-black/55 sm:items-center sm:justify-center">
          ...
          <div className="relative w-full rounded-t-3xl border border-[var(--exa-border)] bg-[var(--exa-surface)] p-5 shadow-[var(--exa-shadow-panel)] sm:max-w-sm sm:rounded-3xl">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold text-[var(--exa-text-primary)]">Link via Address</h3>
              <button type="button" onClick={() => setShowManualModal(false)} className="text-[var(--exa-text-secondary)]">
                <X className="h-4 w-4" />
              </button>
            </div>
            <label className="block">
              <span className="mb-1 block text-xs text-[var(--exa-text-muted)]">Wallet Address</span>
              <input
                value={manualAddress}
                onChange={(event) => setManualAddress(event.target.value)}
                placeholder="0x..."
                className="h-11 w-full rounded-xl border border-[var(--exa-border)] bg-[var(--exa-bg-tertiary)] px-3 text-sm text-[var(--exa-text-secondary)] outline-none transition focus:border-cyan-300/70 focus:shadow-[0_0_18px_rgba(34,211,238,0.2)]"
              />
            </label>
            <button
              type="button"
