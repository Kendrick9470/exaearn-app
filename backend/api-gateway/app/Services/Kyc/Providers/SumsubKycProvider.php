...
class SumsubKycProvider implements KycProviderDriver
{
    public function verifyDocument(array $payload): array
    {
        $data = $this->post('/resources/applicants/-/documents', $payload);

        return [
            'success' => $this->bool($data, ['success', 'status', 'ok']),
            'provider_id' => $this->string($data, ['provider_id', 'id', 'inspectionId']),
    ...
    {
        $data = $this->post('/resources/applicants/-/face', $payload);
    ...
    {
        $data = $this->post('/resources/applicants/-/duplicate', $payload);
    ...
    {
        $data = $this->post('/resources/applicants/-/country', $payload);
    ...
    {
        $cfg = (array) config('kyc.providers.sumsub', []);
        $baseUrl = rtrim((string) ($cfg['base_url'] ?? ''), '/');
        $apiKey = (string) ($cfg['api_key'] ?? '');

        if ($baseUrl === '' || $apiKey === '') {
            throw new RuntimeException('Sumsub KYC provider not configured.');
        }

        $response = Http::timeout(20)
            ->acceptJson()
            ->withHeaders(['X-App-Token' => $apiKey])
